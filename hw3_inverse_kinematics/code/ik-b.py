...
def objective(x):
    ...
    return np.float64(np.square(np.linalg.norm(fk(x, link_lengths) - p_d)))
...
def constraint1(x):
    ...
    output = line_sphere_intersection(p1, p2, obstacle_center, obstacle_radius)
    if output < 0: return -output # if output is negative, no solutions, constraint satisfied
    elif output == 0: return -1 # if output is 0, 1 solution, constraint not satisfied
    return output # if output is positive, 2 solutions, constraint not satisifed
