...
class Particle(object):
    ...
    def UpdateScan(self, scan):
        ...
        #     formulation.
        error = scan.ranges - sim_ranges
        likelihoods = np.exp(-0.5 * error**2 / SENSOR_MODEL_VAR)
...
class ParticleFilter(object):
    ...
    def UpdateScan(self, scan_msg):
        ...
        possible_indices = len(self.particles)
        num_particles_in_next_set = len(self.particles)
        particle_indices = np.random.choice(
            possible_indices,
            size=num_particles_in_next_set,
...
def main():
    ...
    y_err = []
    yaw_err = []
    for topic, msg, t in bag.read_messages(topics=["gt_odom"] + list(subscribers.keys())):
        if topic == "gt_odom":
